...
export default function QuizQuestions(props: Props) {
  ...
  const handleSubmit = async () => {
    try {
      const subId = await saveSubmissions({ score }, props.quiz.id);
    } catch (e) {
      console.log(e);
    }
  ...
  const handlePressPrev = () => {
    if (currentQuestion != 0) {
      setCurrentQuestion(prevCurrenQuestion => prevCurrenQuestion - 1);
    }
  }
            ...
            <div className="grid grid-cols-1 gap-6 mt-6">
              {
                questions[currentQuestion].answers.map
                  (answer => {
                    ...
                    return (
                      <Button key={answer.id} disabled={!!selectedAnswer} variant={variant} size="xl" onClick={() => handleAnswer(answer, questions[currentQuestion].id)} className="disabled:opacity-100"><p
